...
-- 급여 순위 3위~7위까지 조회하기
-- > ROWNUM은 RESULT SET 완성후 적용되는 가상 컬럼
...
-- 사번, 사원명, 전화번호, 고용일, 부서명
SELECT
EMP_ID, EMP_NAME, PHONE, HIRE_DATE, DEPT_TITLE
FROM
EMPLOYEE 
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID) 
...
-- 사번, 이름, 부서코드, 직급코드, 부서명, 직급명
 SELECT 
 	EMP_NO, EMP_NAME,DEPT_CODE,JOB_CODE,DEPT_TITLE
 FROM
 	EMPLOYEE
 JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
 JOIN JOB USING (JOB_CODE)
...
-- 사번, 이름, 부서코드, 직급코드, 고용일
 ...
 SELECT 
 	EMP_NO, EMP_NAME,DEPT_CODE,JOB_CODE,HIRE_DATE
 FROM
 	EMPLOYEE
 WHERE
 	HIRE_DATE = (
 	...
 	FROM
 		EMPLOYEE
 	WHERE
 		EXTRACT(YEAR FROM HIRE_DATE) LIKE '2010');
...
WHERE (DEPT_CODE,MANAGER_ID) =
	( SELECT
		...
		EMPLOYEE
		WHERE
		SUBSTR(EMP_NO,1,2) LIKE '87%'
		AND
		SUBSTR(EMP_NO,8,1) LIKE '2%'
