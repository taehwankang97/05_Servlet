...
FROM DUAL; -- 3 (INDEX 아님, 1부터 시작하는 COUNT개념)

-- 문자열 5번째 부터 검색 시작해서 처음 찾은 'B'의 위치 조회 

SELECT 'AABAACAABBAA', 
			 INSTR('AABAACAABBAA', 'B',5)
FROM DUAL; -- 9 번째 

-- 문자열 5번째 부터 검색 시작해서 두번째 찾은 'B'의 위치 조회 
...
-- CEIL(숫자 | 컬럼명)  : 올림 -> 정수형태로 반환 
-- FLOOR(숫자 | 컬럼명) : 내림 -> 정수형태로 반환 
...
SELECT 123.456, 
ROUND(123.456), -- 123
ROUND(123.456,0),--123 (0이 라운드 기본 값)
...
FROM EMPLOYEE
--WHERE HIRE_DATE BETWEEN '2010-01-01' AND '2019-12-31'
WHERE MOD(EXTRACT(YEAR FROM HIRE_DATE),2)=0 BETWEEN 2010 AND 2019
ORDER BY  HIRE_DATE DESC; 
...
--	TO_CHAR(SALARY * 12, '999,999,999L') 연봉 뒤에는 안됨
FROM EMPLOYEE;
...
SELECT TO_CHAR(CURRENT_DATE,'YY"년" MM"월" DD"일" PM HH"시" MI"분" SS"초"')
FROM DUAL;
...
FROM DUAL;
-- TO_DATE에 매개변수가 한 개만 작성될 수 있는 경우 
--> () 내부 문자열이 날짜/시간 일반적인 형식일 경우에만 사용가능

-- 일반적인 형식이 아닌 경우 포맷 지정이 필수
...
SELECT SUM(SALARY)
FROM EMPLOYEE -- 데이터내 모든 행중 
WHERE  DEPT_CODE = 'D6';

-- AVG(숫자만 기록된 컬럼) : 급여 평균
...
SELECT 
	...
	MIN(EMP_NAME) "제일 먼저 작성된 이름",
	MAX(EMP_NAME) "마지막에 작성된 이름"
FROM EMPLOYEE;
