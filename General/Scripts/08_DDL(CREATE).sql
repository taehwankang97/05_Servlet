...
/* 자료형
    ...
    CHAR(크기) : "고정"길이 문자형 (2000BYTE) 
        ...
        --> 주민번호, 전화 번호와 같이 고정된것에 사용
        
    VARCHAR2(크기) : "가변"길이 문자형 (4000 BYTE)
        -> ex) VARCHAR2(10) 컬럼에 'ABC' 3BYTE 문자열만 저장하면 나머지 7BYTE를 반환함.
        --> 이름이나 통장 잔고와 같이 고정되어 있지 않고 사용자에 따라 달라지는 것 사용
        ---> 나머지를 반환 하기 때문에 시간이 2배 걸림
        
         == VATCHAR2 주의 사항==
         오라클은 기본적으로 UTF-8 문자 인코딩을 사용하는데
         UTF-8은 영어/숫자 1BYTE, 한글 3BYTE로 저장하기 때문에
         한글 데이터 저장 시 크기 계산을 주의 
         
         NVARCHAR2(크기) : "가변" 길이 묹자형 (4000BYTE)
         -> BYTE 크기가 아닌 글자 수 (문자 개수로) 크기를 지정하는 타입 
         
         EX) VARCHAR2(3) : 가변길이 문자형 크기 3BYTE
         		 NVARCHAR(3) : 가변길이 문자형 문자 3개 까지 저장 가능 
    DATE : 날짜 타입
    	TIMESTAMP : 날짜 타입 (MS 단위 + UTC기준 시차)
    
    BLOB : 대용량 이진 데이터 (4GB) // 블로그에 웹 소설을 올리는 것과 같은 특수한 상화에만 사용
    CLOB : 대용량 문자 데이터 (4GB) // 
*/

-- MEMBER 테이블 생성

-- 필요한 컬럼 : 아이디, 비밀번호, 이름, 주민등록 번호 , 가입일
...
COMMENT ON COLUMN "MEMBER".MEMBET_ID IS '회원 아이디';
COMMENT ON COLUMN "MEMBER".MEMBET_PW IS '회원 비밀번호';
COMMENT ON COLUMN "MEMBER".MEMBET_NAME IS '회원 이름';
COMMENT ON COLUMN "MEMBER".MEMBET_SSN IS '회원 비밀번호';
COMMENT ON COLUMN "MEMBER".MEMBET_DATE IS '회원 가입일';
...
VALUES('MEM01', '123ABC', '동길홍', '990102-1234567', CURRENT_DATE);
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE(옛날 사용)/CURRENT(최근 주로 사용)를 활용
INSERT INTO "MEMBER"
VALUES('MEM02', 'QWER1234', '희영김', '980808-2345678', CURRENT_DATE);
...
/*
    ...
    제약조건 종류
    PRIMARY KEY
     각 행을 구별할 수 있게 식별자 역활을 하는 역활을 가진 컬럼
     중복 X (unique) + 무조건 값이 존재 (NOT NULL)
     -PK는 테이블에 1개만 존재 가능 
     (한컬럼 X PK 제약조건이 1개 )
    , NOT NULL, UNIQUE, CHECK, 
    FOREIGN KEY
    자식테이블의 한 컬럼에 작성될 수 있는 값은
    부모 테이블의 한 컬럼에 작성된 값만 쓸 수 있다 
    --> 자식 테이블이 부모 테이블을 참조.
...
-- 컬럼 레벨 : 테이블 생성 시 컬럼의 정보를 작성하는 부분
...
-- 단, UNIQUE 제약 조건이 설정된 컬럼에 NU LL 값은 중복 삽입 가능.


-- UNIQUE 제약 조건 테이블 생성
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
    --USER_ID VARCHAR2(20) UNIQUE, --컬럼 제약 조건(제약 조건명 X) 
  	USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE,
    									-- CONSTRAINT 제약조건명   제약조건 종류 
    									--> 해당 컬럼에 "USER_ID_U" 라는 이름의 
    									-- 	UNIQUE 제약 조건을 생성(설정)
    USER_PWD VARCHAR2(30) ,
    USER_NAME VARCHAR2(30), -- 테이블 레벨로 제약조건 설명하기 
    GENDER VARCHAR2(10),
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    
    /* 테이블 레벨 : 테이블 생성구문에서 컬럼 정의가 끝난 이후 부분
     *  - 제약조건종류 (컬럼) : 어떤 컬럼에 제약조건을 설정할지 작성
     * */
   -- UNNIQUE(USER_NAME) -- 테이블 레벨 제약 조건 (제약 조건명 X)
...
INSERT INTO USER_USED_UK
VALUES(1, NULL, 'pass01', '고길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
--> 아이디에 NULL 값 삽입 가능.
...
INSERT INTO USER_USED_UK
VALUES(1, NULL, 'pass01', '나길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
--SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KTH.USER_NAME_U)에 위배됩니다
...
TODOLIST_NO NUMBER,
TODOLIST_TODO VARCHAR(30),
TODOLIST_C CHAR(3),
ENROLL_DATE DATE DEFAULT CURRENT_DATE,
TODOLIST_DETAIL VARCHAR(4000)
);
SELECT
	*
FROM
...
CREATE TABLE USER_USED_PK(
    USER_NO NUMBER CONSTRAINT USER_NO_PK 
    /*어디서 오류가 났는지 이름 정해주기 */
    PRIMARY KEY, -- 컬럼 레벨 설정 
...
-- 제공되는 값 외에는 NULL을 사용할 수 있음 -- 참조하는게 없다 라고 볼 수 있다
...
-- 자식 테이블 : 참조를 "하는" 테이블 (USER_USED_KF)
...
/* 참조를 당하고 있는 값(GRADE_CODE) 10 삭제하기 */
DELETE
FROM USER_GADE
WHERE GRAD_CODE = 10;
...
CREATE TABLE USER_TEST(
  ...
  STATUS VARCHAR2(1) CONSTRAINT CK_STATUS
  	CHECK (STATUS IN ('Y', 'N'))*/
