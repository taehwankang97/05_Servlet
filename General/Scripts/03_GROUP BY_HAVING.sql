...
-- 부서코드 순으로 정렬
SELECT  -- 조회 결과에 포함 될 컬럼 지정 
DEPT_CODE,
COUNT(*)
FROM -- 어떤 테이블에서
EMPLOYEE  
WHERE BONUS IS NOT NULL -- 어떤 행만 
GROUP BY DEPT_CODE     -- WHERE 경과에서 그룹 구문
ORDER BY DEPT_CODE ASC; -- 조회 결과 정렬
...
-- 부서 코드 오름차순으로 정렬
SELECT
DEPT_CODE,JOB_CODE,SUM(SALARY), COUNT(*)
FROM
EMPLOYEE
GROUP BY
DEPT_CODE, JOB_CODE -- 1) 부서별 그룹 후 2) 직급별 그룹
ORDER BY
DEPT_CODE ASC;
...
DEPT_CODE
ORDER BY DEPT_CODE ASC;
--  개인별 급여가 350만 이상인 사람들 (틀린 구문 잘못된 SQL)
...
ORDER BY DEPT_CODE ASC;
--> EMPLOYEE 테이블에서 DEPT_CODE 기준으로 만들어진 그룹중 
...
JOB_CODE
ORDER BY
JOB_CODE ASC; -- 4행 조회 (0명은 조회 결과에 포함 X)
...
--급여가 400 만 초과하는 사원의 이름, 부서코드, 급여 조회
SELECT
EMP_NAME, DEPT_CODE, SALARY
FROM
EMPLOYEE
