...
-- DEPARTMENT2 테이블에 모든 부서명을 '인사팀'으로 수정 
...
FROM 
	EMPLOYEE2
WHERE 
	EMP_NAME IN ('유재식','노옹철', '전형돈', '정중하', '하동운');
...
-- 1) 아시아 지역에 근무하는 직원
	SELECT 
		EMP_NAME, LOCAL_NAME, BONUS
	FROM
	EMPLOYEE2
	JOIN 
		DEPARTMENT2 ON (DEPT_CODE = DEPT_ID)
	JOIN
		LOCATION ON (LOCAL_CODE = LOCATION_ID)
	WHERE 
...
-- TRUNCATE로 삭제
TRUNCATE TABLE EMPLOYEE3; -- 테이블 삭제 x 행만 전체 삭제 
